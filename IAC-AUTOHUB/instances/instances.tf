...
resource "aws_instance" "ec2_publica" {
  ...
  key_name        = aws_key_pair.autohub_chave.key_name
  ...
  user_data = <<-EOF2
            ...
            sudo docker run -p 3000:80 -d matteusnogueira/autohub-front:latest


          EOF2

  tags = {
    Name = "AUTOHUB-PUBLICA"
  }
}
...
resource "aws_instance" "ec2_privada" {
  ...
  user_data = <<-EOF
            ...
            sudo docker run -p 8080:8080 -d matteusnogueira/autohub-backend:latest
          EOF

  tags = {
    Name = "AUTOHUB-PRIVADA"
  }
}
...
resource "aws_key_pair" "autohub_chave" {
  key_name   = "autohub_chave"
  public_key = tls_private_key.ssh_key.public_key_openssh
}

resource "local_file" "private_key" {
  content         = tls_private_key.ssh_key.private_key_pem
  filename        = "${path.module}/../keys/autohub_chave.pem"
...
output "private_instance_id_2" {
  ...
  description = "ID da instância EC privado 2"
}
